...
async function kaforaListProfiles(limit = 10) {
  if (_dbReady) {
    try {
      const snap = await _db.collection('users').limit(limit).get();
      const profiles = [];
      for (const userDoc of snap.docs) {
        const u = userDoc.data();
        const profSnap = await _db.collection('users').doc(userDoc.id).collection('profile').doc('main').get();
        if (profSnap.exists) {
          ...
          profiles.push({
            ...
            city: d.city,
            bio: d.bio,
            accentColor: d.accentColor || '#B08D57'
          });
        }
      }
      return profiles;
    } catch(e) {
      console.warn('[kafora-db] List profiles error:', e.message);
    }
  }
  // Fallback : return current local profile as a list item
  const p = JSON.parse(localStorage.getItem('kafora-profil') || '{}');
  const u = JSON.parse(localStorage.getItem('kafora-user') || '{}');
  if (p.photo) {
    return [{
      uid: 'demo',
      displayName: u.displayName || 'Demo Artist',
      photo: p.photo,
      discs: p.discs || [],
      city: p.city || 'Paris',
      bio: p.bio,
      accentColor: p.accentColor || '#B08D57'
    }];
  }
  return [];
}
